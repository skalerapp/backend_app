...
const listAssets = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error al listar activos de almacen', error: error.message });
  }
};
...
const importAssets = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: 'Error al importar activos de almacen', error: error.message });
  }
};
...
const createAsset = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: 'Error al registrar activo de almacén', error: error.message });
  }
};
...
const listMovements = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error al listar movimientos de almacen', error: error.message });
  }
};
...
const createMovement = async (req, res) => {
  try {
    ...
    if (isFleetMovement) {
      ...
      if (!plateSnapshot) {
        connection.release();
        return res.status(400).json({ success: false, message: 'La flota requiere placa o identificación de unidad para registrar el movimiento' });
      }
      if (!(odometer_snapshot || '').toString().trim()) {
        connection.release();
        return res.status(400).json({ success: false, message: 'La flota requiere odómetro u horómetro para registrar la salida' });
      }
      if (!(fuel_level_snapshot || '').toString().trim()) {
        connection.release();
        return res.status(400).json({ success: false, message: 'La flota requiere nivel de combustible o carga al momento de la entrega' });
      }
      if (!(checklist_snapshot || '').toString().trim()) {
        connection.release();
        return res.status(400).json({ success: false, message: 'La flota requiere checklist de salida o entrega' });
      }
      ...
      if (missing.length) {
        ...
        return res.status(409).json({
          success: false,
          message: `No se puede mover la flota porque falta vigencia de ${missing.map((item) => item.label).join(', ')}`,
        });
      }
      ...
      if (expired.length) {
        ...
        return res.status(409).json({
          success: false,
          message: `No se puede mover la flota porque tiene vencido ${expired.map((item) => item.label).join(', ')}`,
        });
      }
    }
    ...
    res.status(500).json({ success: false, message: 'Error al registrar movimiento de almacen', error: error.message });
  }
};
